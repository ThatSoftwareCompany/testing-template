...
run_update_test() {
	...
	find "$directory" -type f -name '*.go' -print0 | while IFS= read -r -d '' file; do
		sed -i 's|github.com/ThatSoftwareCompany/testing-template|github.com/example/update-derived|g' "$file"
	done
...
run_legacy_bootstrap_test() {
	...
	find "$directory" -type f -name '*.go' -print0 | while IFS= read -r -d '' file; do
		sed -i 's|github.com/ThatSoftwareCompany/testing-template|github.com/example/legacy-derived|g' "$file"
	done
	git -C "$directory" add .
